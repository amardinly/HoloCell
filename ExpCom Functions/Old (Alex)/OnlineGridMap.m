...
%choose a random subset of frames to load into front of DMD
    ...
    ExpStruct.JGM.playStart = ExpStruct.JGM.buffSize;
...
if ExpStruct.JGM.bufferloadcounter>=(ExpStruct.JGM.buffSize/ExpStruct.JGM.loadsPertrial); 
    ...
    currentMin=min(ExpStruct.JGM.trialsPersite(thisPool));
%     currentPool=find(ExpStruct.JGM.trialsPersite(thisPool)<currentMax);
    currentMin=min(ExpStruct.JGM.trialsPersite(thisPool));
    currentPool=find(ExpStruct.JGM.trialsPersite(thisPool)==currentMin);
...
ExpStruct.postSweepprogramChoice='OGMplaySelection';
