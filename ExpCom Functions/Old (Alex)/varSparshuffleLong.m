...
for j = 1:10
    ...
    if thisSpchoose>numSpar
        ExpStruct.shuffle{ExpStruct.sweep_counter}(j)=1;
    else
        ExpStruct.shuffle{ExpStruct.sweep_counter}(j)=0;
    end

    ExpStruct.spchoose{ExpStruct.sweep_counter}(j)=thisSpchoose-ExpStruct.shuffle{ExpStruct.sweep_counter}(j)*22;
    ...
    %that sparseness, don't repeat same pick if shuffling
        if ExpStruct.shuffle{ExpStruct.sweep_counter}(j)==0
            thisFrameorder= [thisFrameorder ones(1,pulsesPerstim)*randi(framesPerspar,1)+thisOffset];
        elseif ExpStruct.shuffle{ExpStruct.sweep_counter}(j)==1
            thisFrameorder= [thisFrameorder randperm(framesPerspar,pulsesPerstim)+thisOffset];
        end
