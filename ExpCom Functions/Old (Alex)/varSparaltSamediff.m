...
%load up front of buffer if first trial
    ...
    for j=1:length(ExpStruct.currentsparseness)        
        ...
        ExpStruct.currentStimtoLoad{i+(j-1)*20}=y;
        end
    end
...
%alternate 10 frames same sparseness or same frame 10x

    
    
ExpStruct.spchoose{ExpStruct.sweep_counter}=randi(22,20,1);
ExpStruct.sparseness{ExpStruct.sweep_counter}=ExpStruct.lastsparseness(ExpStruct.spchoose{ExpStruct.sweep_counter});

thisframeorder=[];
for i = 1:10
    thisframeorder=[thisframeorder; randi(20,1)+ExpStruct.playstart+(ExpStruct.spchoose{ExpStruct.sweep_counter}(i*2-1)-1)*10];
    thisframeorder = [thisframeorder; randi(20,10,1)+ExpStruct.playstart+(ExpStruct.spchoose{ExpStruct.sweep_counter}(i*2)-1)*10];
end
